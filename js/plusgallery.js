    ...
    var pg = {
      ...
      getDataAttr: function() {
        ...
        if (dataAttr) {
          ...
          var titleAttr = lmnt.data('album-title');
      ...
      writeHTML: function() {
        ...
        $('body').prepend(
          '<div id="pgzoomview" class="pg '+touchClass+'">'+
            '<a href="#" rel="previous" id="pgzoomclose" title="'+pg.translations['zoomview btn close']+'">'+pg.translations['zoomview btn close']+'</a>'+
            '<a href="#" rel="previous" id="pgprevious" class="pgzoomarrow" title="'+pg.translations['zoomview btn previous']+'">'+pg.translations['zoomview btn previous']+'</a>'+
            '<a href="#" rel="next" id="pgnext" class="pgzoomarrow" title="'+pg.translations['zoomview btn next']+'">'+pg.translations['zoomview btn next']+'</a>'+
            '<div id="pgzoomscroll">'+
            '<ul id="pgzoom"></ul>'+
            '</div>'+
            '</div>'
        ...
        if (pg.credit === true) {
          lmnt.append('<div id="pgcredit"><a href="http://www.plusgallery.net" target="_blank" title="'+pg.translations['powered by']+' +GALLERY"><span>+</span>Gallery</a></div>');
        }
          ...
          if (pg.type == 'local') {
            $('#pgalbums').append(
              '<li class="pgalbumthumb">'+
                '<a href="#" data-album-index="'+galleryJSON+'" class="pgalbumlink">'+
                  imgHTML+
                  '<span class="pgalbumtitle">'+galleryTitle+'</span>'+
                  '<span class="pgplus">+</span>'+
                '</a>'+
              '</li>'
            );
          }
            ...
            $('#pgalbums').append(
              '<li class="pgalbumthumb">' +
                '<a href="'+galleryJSON+'" class="pgalbumlink">'+
                  imgHTML+
                  '<span class="pgalbumtitle">'+galleryTitle+'</span>'+
                  '<span class="pgplus">+</span>'+
                '</a>'+
      ...
      parseData: function(json, title) {
        ...
        switch(pg.type) {
          case 'google':
            galleryTitle = json['feed']['title']['$t'];
            objPath = json['feed']['entry'];
            break;
          case 'flickr':
            galleryTitle = json['feed']['title']['_content'];
            objPath = json['photoset']['photo'];
            break;
          case 'facebook':
            galleryTitle = json['data']['name'];
            objPath = json['data'];
            break;
          case 'instagram':
            objPath = json['data'];
        ...
        $('.crumbtitle').remove();
        $('#pgthumbs').empty();
        if (typeof(title) === typeof(undefined) && galleryTitle.length > 0) {
          title = galleryTitle;
        }
        else if (typeof(title) === typeof(undefined) || title.length == 0) {
          title = '&nbsp;';
        }
          ...
          if (i < pg.limit) {
            ...
            $('#pgthumbs')
              .append(
                '<li class="pgthumb">'+
                  '<a href="'+imgSrc+'">'+
                    '<img src="'+imgTh+'" id="pgthumbimg'+i+'" class="pgthumbimg" alt="'+imgTitle+'" title="'+imgTitle+'"'+imgBg+'>'+
                  '</a>'+
                '</li>'
              );
      ...
      loadZoom: function(idx) {
        ...
        pgZoomView.fadeIn(function() {
          ...
          $.each(pg.imgArray, function(i) {
            pgZoomHTML =
              pgZoomHTML+
              '<li class="pgzoomslide loading" id="pgzoomslide'+i+'" style="width: '+pg.winWidth+'px;">'+
                '<img src="'+pg.imagePath+'/square.gif" class="pgzoomspacer">'+
                '<span class="pgzoomcaption">'+pg.titleArray[i]+'</span>'+
              '</li>';
      ...
      loadZoomImg: function(idx) {
        if ($('#pgzoomimg'+idx).length === 0) {
          var img = $(
            ...
            {
              'src': pg.imgArray[idx],
              'data-src': pg.imgArray[idx],
              'title': pg.titleArray[idx],
              'alt': pg.titleArray[idx],
              'id': 'pgzoomimg'+idx,
              'class': 'pgzoomimg'
            }
          );
          $('#pgzoomslide'+idx+' .pgzoomspacer').after(img);
          img.load(function() {
      ...
      prevNext: function(dir) {
        ...
        currentIdx = pgzoomscroll.scrollLeft() / pg.winWidth;
